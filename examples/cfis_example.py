...
from wl_stats_torch.visualization import plot_comparison, plot_l1_norms, plot_peak_histograms
...
def main():
    ...
    im0 = axes[0, 0].imshow(
        kappa_true.cpu().numpy(), cmap="RdBu_r", origin="lower", vmin=-0.05, vmax=0.05
    )
    axes[0, 0].set_title("True Convergence Field")
    plt.colorbar(im0, ax=axes[0, 0])

    im1 = axes[0, 1].imshow(
        kappa_noisy.cpu().numpy(), cmap="RdBu_r", origin="lower", vmin=-0.05, vmax=0.05
    )
    axes[0, 1].set_title("Noisy Convergence Field")
    plt.colorbar(im1, ax=axes[0, 1])

    im2 = axes[1, 0].imshow(
        (kappa_noisy * mask).cpu().numpy(), cmap="RdBu_r", origin="lower", vmin=-0.05, vmax=0.05
    )
    ...
    stats = WLStatistics(n_scales=N_SCALES, device=device, pixel_arcmin=PIX_ARCMIN)
